...
import 'package:gomeat/Const/Constants.dart';
...
class APIHelper {
  ...
  Future<dynamic> addToCart(int qty, int varientId, int special) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        print("aaaaaaaaaaa : "+response.data['data'].toString());
  ...
  Future<dynamic> getCartList() async {
    try {
      ...
      if (response.statusCode == 200 && response.data["status"] == '1') {
        recordList = Cart.fromJson(response.data['data']);
        print("Aaaaaiddd : "+global.currentUser.id.toString());
        print("Aaaaa : "+response.data['data'].toString());
      } else {
        recordList = null;
      }
      return getDioResult(response, recordList);
  ...
  Future<dynamic> getProductDetail(int productId) async {
    try {
      Response response;
      var dio = Dio();
      var formData = FormData.fromMap({"product_id": "3463"});
  ...
  getSubcategoryProduct(int catId,String mainCatId, int page, ProductFilter productFilter) async {
    try {
      ...
      print("aaaaaaaaaaaa : "+catId.toString());
      print("aaaaaaaaaaaa : "+global.currentUser.id.toString());
