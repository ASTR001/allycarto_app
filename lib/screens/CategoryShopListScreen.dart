...
import 'Shop/shop_view_page.dart';

class CategoryShopListScreen extends StatefulWidget {
  final String catId;
  final String name;

  const CategoryShopListScreen({Key key, this.catId, this.name}) : super(key: key);

  @override
  State<CategoryShopListScreen> createState() => _CategoryShopListScreenState();
}

class _CategoryShopListScreenState extends State<CategoryShopListScreen> {
  ...
  Future<String> getShopData() async {
    try {
      final response = await http.get(Uri.parse(global.baseUrl+"category_vendor?cat_id="+widget.catId+"&pincode=600034"),);
