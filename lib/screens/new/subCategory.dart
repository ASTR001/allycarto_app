...
class SubCategoryScreen extends BaseRoute {
  final String catId;
  final String name;

  SubCategoryScreen(this.catId, this.name, {a, o}) : super(a: a, o: o, r: 'SubcategoryListScreen');
...
class _SubcategoryListScreenState extends BaseRouteState {
  ...
  Future<String> getShopData() async {
    try {
      final response = await http.get(Uri.parse(global.baseUrl+"subcategory/"+catId),);
              ...
              child: new Card(
                child: InkWell(
                  onTap: () {
                    ...
                    Navigator.of(context).push(
                      MaterialPageRoute(
                        builder: (context) => ProductListScreen(1, _shopList[i]["title"], catId.toString(), subcategoryId: _shopList[i]["cat_id"], a: widget.analytics, o: widget.observer),
                      ),
                    );
                  },
                  child: Container(
